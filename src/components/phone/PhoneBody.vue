<template>
  <div class="phone-body" :class="{ 'dark': appearance.darkMode }" ref="phoneBodyRef"
    @contextmenu.stop="handlePhoneBodyContextMenu">
    <div class="wechat-content">
      <div class="wechat-item" :id="chat.id" v-for="chat in useChatStore.chatList" :key="chat.id"
        :class="{ 'wechat-item-right': chat.role === 'own', 'wechat-item-rejected': chat.role === 'own' && chat.rejected, 'wechat-item-notice-box': !showAvatar(chat), 'active': useContextMenuStore.activeChatId === chat.id }"
        @contextmenu.stop="e => rightClicked(e, chat.id)">
        <div class="wechat-item-avatar" v-if="showAvatar(chat)">
          <img :src="chat.user.avatar" alt="">
        </div>
        <div>
          <div class="wechat-item-name"
            v-if="useSystemStore.appearance.showChatName && !['time', 'takeAPat', 'revoke', 'system'].includes(chat.type)">
            {{ chat.user.nickname }}
          </div>
          <div class="wechat-item-text" v-if="chat.type === 'text'" v-html="renderText(chat.content, emojiBase64)">
          </div>
          <div class="wechat-item-text wechat-item-image" v-else-if="chat.type === 'image'">
            <img :src="chat.content" alt="">
          </div>
          <div class="wechat-item-text wechat-item-trans" :class="{ 'wechat-item-trans-received': chat.received }"
            v-else-if="chat.type === 'transferAccounts'">
            <div class="wechat-item-trans-content">
              <i></i>
              <div>
                <span>¥{{ chat.money.toFixed(2) }}</span>
                <span class="font" v-if="chat.received">已被领取</span>
                <span class="font" v-else-if="!chat.content">转账给{{ chat.role === 'own' ?
                  useUserStore.activeUser.nickname : "你" }}</span>
          ...
          <div class="wechat-item-text wechat-item-trans" :class="{ 'wechat-item-trans-received': chat.received }"
            v-else-if="chat.type === 'redEnvelope'">
          ...
          <div class="wechat-item-notice"
            v-else-if="chat.type === 'receive' && chat.receivedChatType === 'redEnvelope'">
            <i></i>{{ chat.role === 'own' ? "你" : useUserStore.activeUser.nickname }}领取了{{ chat.role === 'own' ?
              useUserStore.activeUser.nickname : "你" }}的<em>红包</em>
          </div>
          <div class="wechat-item-text wechat-item-trans wechat-item-trans-received"
            v-else-if="chat.type === 'receive' && chat.receivedChatType === 'transferAccounts'">
            <div class="wechat-item-trans-content">
              <i></i>
              <div>
                <span>¥{{ chat.money.toFixed(2) }}</span>
          ...
          <div class="wechat-item-voice-wrapper" v-else-if="chat.type === 'voice'">
            <div class="wechat-item-text wechat-item-voice" @click="handleVoiceClick(chat.duration, $event)">
              <i :ref="(el) => voiceIcons[chat.id] = el"></i>
              <span>{{ chat.duration }}"</span>
              <div :style="{ width: chat.duration * 5 + 'px' }"></div>
              <em v-if="!chat.received"></em>
            </div>
          ...
          <div class="wechat-item-notice bg" :class="{ 'bold': chat.type === 'takeAPat' && chat.patBold }"
            v-else-if="['time', 'takeAPat', 'revoke', 'system'].includes(chat.type)">
...
const handleVoiceClick = (duration, event) => {
  console.log('语音消息被点击，时长：', duration);

  // 获取当前语音消息的ID
  const chatId = event.currentTarget.closest('.wechat-item').id;
  const iconElement = voiceIcons[chatId];
  ...
  if (animationInterval) {
    ...
    // 移除所有样式
    for (let i = 1; i <= 3; i++) {
      iconElement.classList.remove(`wechat-item-voice-i-${i}`);
    }
  }
  ...
  animationInterval = setInterval(() => {
    ...
    // 根据模式添加当前样式
    const className =
      `wechat-item-voice-i-${currentClass}`;

    iconElement.classList.add(className);

    // 更新当前样式
    currentClass = currentClass % 3 + 1;
  }, 300);
  ...
  .wechat-content {
    .wechat-item {
      ...
      .wechat-item-text {
        ...
        // 语音动画图标样式 - 浅色模式
        .wechat-item-voice-i-1 {
          width: 34px !important;
          height: 49px !important;
          background: url(@/assets/images/content/wechat-voice-icon1-light-1.png) no-repeat !important;
        }

        .wechat-item-voice-i-2 {
          width: 34px !important;
          height: 49px !important;
          background: url(@/assets/images/content/wechat-voice-icon1-light-2.png) no-repeat !important;
        }

        .wechat-item-voice-i-3 {
          width: 34px !important;
          height: 49px !important;
          background: url(@/assets/images/content/wechat-voice-icon1-light.png) no-repeat !important;
        }
      ...
      &.wechat-item-right {
        ...
        .wechat-item-text {
          ...
          .wechat-item-voice-i-1 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon2-light-1.png) no-repeat !important;
          }

          .wechat-item-voice-i-2 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon2-light-2.png) no-repeat !important;
          }

          .wechat-item-voice-i-3 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon2-light.png) no-repeat !important;
          }
      ...
      .wechat-item {
        ...
        .wechat-item-text {
          ...
          .wechat-item-voice-i-1 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon1-dark-1.png) no-repeat !important;
          }

          .wechat-item-voice-i-2 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon1-dark-2.png) no-repeat !important;
          }

          .wechat-item-voice-i-3 {
            width: 34px !important;
            height: 49px !important;
            background: url(@/assets/images/content/wechat-voice-icon1-dark.png) no-repeat !important;
          }
